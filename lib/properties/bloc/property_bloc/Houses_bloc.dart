...
import 'Houses_event.dart';

class HouseBloc extends Bloc<HouseEvent, HouseState> {
  final _firestore = FirebaseFirestore.instance;
  static House? house;
  HouseRepositry groupsRepositry = HouseRepositry();
  HouseBloc() : super(HouseInithal()) {
    on<LoadHouses>((event, emit) async {
      emit(HouseLoading());
      try {
        var houses = groupsRepositry.getHouseStream();
        emit(HouseLoaded(Houses: houses));
      } catch (e) {
        emit(HouseError(message: 'Error loading Houses: $e'));
      }
    });
    on<AddHouse>((event, emit) async {
      emit(HouseLoading());
      try {
        await groupsRepositry.addHouse(event);
        var houses = await groupsRepositry.getHouseStream();
        emit(HouseLoaded(Houses: houses));
      } catch (e) {
        emit(HouseError(message: 'Error adding House: $e'));
      }
    });
    on<DeleteHouse>((event, emit) async {
      emit(HouseLoading());
      try {
        await groupsRepositry.deleteHouse(event.houseId);
        var houses = await groupsRepositry.getHouseStream();
        emit(HouseLoaded(Houses: houses));
      } catch (e) {
        emit(HouseError(message: 'Error adding House: $e'));
  ...
  Stream<List<House>> get housesStream => groupsRepositry.getHouseStream();
  String search = "";
  Stream<List<House>> housesStreamByName(String searchQuery) {
    return groupsRepositry.getHouseStreamByName(search);
  }
}
...
/*
  Stream<HouseState> mapEventToState(GroupEvent event) async* {
